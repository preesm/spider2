...
void envInit(float *state_angular, float *state_observation) {
    ...
    float sign = static_cast<float>(randomSign());
    state_angular[0] = sign * static_cast<float>(M_PI) * rand_norm_float();
    sign = static_cast<float>(randomSign());
    state_angular[1] = sign * rand_norm_float();
    ...
    state_observation[0] = static_cast<float>(cos(static_cast<double>(state_angular[0])));
    state_observation[1] = static_cast<float>(sin(static_cast<double>(state_angular[0])));
...
void step(int state_space_size, int action_space_size,
    ...
    if (timestep % REWARD_SIZE == 0 && timestep > 0) {
        ...
        mean_reward /= static_cast<float>(REWARD_SIZE);
...
//    }
    ...
    // Compute reward
    if (reward) {
        const auto scaled_angular_speed = 0.1 * pow2(static_cast<double>(angular_speed));
        const auto scaled_action_clip = 0.001 * (pow2(static_cast<double>(action_clip)));
        const auto scaled_theta = pow2(modf((static_cast<double>(theta) + M_PI), (2. * M_PI)) - M_PI);
        reward[0] = static_cast<float>(scaled_theta + scaled_angular_speed + scaled_action_clip);
        reward[0] = -(reward[0]);
    ...
    angular_speed = angular_speed + ((-3.f) * G_CONSTANT / (2.f * LENGTH_CONSTANT) * 
                                     static_cast<float>(sin(theta + static_cast<float>(M_PI))) +
    ...
    state_observation[0] = static_cast<float>(cos(static_cast<double>(theta)));
    state_observation[1] = static_cast<float>(sin(static_cast<double>(theta)));
